...
from UNET_RES import WHOLE_NET
...
def get_args():
    ...
    parser.add_option('-r', '--root', dest='root', default="/home/lym_pcl/lym_dataset/Nguyen_1523/", help='root directory')
  ...
  #  parser.add_option('-w', '--w1', dest='w1', default=0, type='float', help='loss_w1')
    ...
    parser.add_option( '--w3', dest='w3', default=1, type='float',help='folder of ground truth')
    parser.add_option( '--w4', dest='w4', default=500, type='float',help='folder of ground truth')    
      # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!必改#生成权重的文件 
    parser.add_option('-m', '--model', dest='model', default='Result/Nguyen_760/Whholenet/ResUnet', help='folder for model/weights')
    parser.add_option('--wandb_project_name',dest='wandb_project_name',default='Nguyen_760')  
...
def setup_and_run_train(load_weights,dir_input1,dir_gt_fenzi, dir_gt_fenmu, dir_gt_wrap, dir_gt_unwrap, dir_model,wandb_project_name, val_perc, batch_size, epochs,lr,log_name1,w1,w2,w3,w4,test_size=0.1):
    ...
    header = ['epoch', 'learning rate', 'train loss_total','val loss_total', 'a1_ave','a2_ave','a3_ave','time cost now/second',
        'train loss_unwrap','val loss_unwrap', 'train loss_fenzi','val loss_fenzi','train loss_fenmu','val loss_fenmu', 'train loss_wrap','val loss_wrap' ,
    ...
    for epoch in tqdm(range(epochs)):#tqdm显示进度条，epoche代表循环的次数
        ...
        train_loss_fenzi,output_train_fenzi,train_loss_fenmu,output_train_fenmu,train_loss_wrap,\
        ...
        scheduler.step()
        val_loss_fenzi,output_val_fenzi,val_loss_fenmu,output_val_fenmu,val_loss_wrap,\
...
#用wandb 对训练的结果进行画图
        ...
        output_train_fenzi= wandb.Image(output_train_fenzi, caption="epoch:{}".format(epoch))  # attention!!!
        output_train_fenmu= wandb.Image(output_train_fenmu, caption="epoch:{}".format(epoch))  # attention!!!
        wandb.log({"output_train_unwrap": output_train_unwrap,'epoch':epoch})
        wandb.log({"output_train_wrap": output_train_wrap,'epoch':epoch})
        wandb.log({"output_train_fenzi": output_train_fenzi,'epoch':epoch})
        wandb.log({"output_train_fenmu": output_train_fenmu,'epoch':epoch})
        #对训练的结果画损失函数
        ...
        wandb.log({'train_loss_fenzi': train_loss_fenzi,"epoch":epoch})   
        wandb.log({'train_loss_fenmu': train_loss_fenmu,"epoch":epoch})                   
...
#用wandb 对测试的结果进行画图               
        ...
        output_val_fenzi= wandb.Image(output_val_fenzi, caption="epoch:{}".format(epoch))  # attention!!!
        output_val_fenmu= wandb.Image(output_val_fenmu, caption="epoch:{}".format(epoch))  # attention!!!
        wandb.log({"output_val_unwrap": output_val_unwrap,'epoch':epoch})
        wandb.log({"output_val_wrap": output_val_wrap,'epoch':epoch})
        wandb.log({"output_val_fenzi": output_val_fenzi,'epoch':epoch})
        wandb.log({"output_val_fenmu": output_val_fenmu,'epoch':epoch})
        #对训练的结果画损失函数
        ...
        wandb.log({'val_loss_fenzi': val_loss_fenzi,"epoch":epoch})   
        wandb.log({'val_loss_fenmu': val_loss_fenmu,"epoch":epoch})                   
        ...
        values = [epoch+1, round(scheduler.get_last_lr()[0],10), '%.6f' % train_loss_total, '%.6f' % val_loss_total,\
                  ...
                  '%.6f' % train_loss_unwrap,'%.6f' % val_loss_unwrap,'%.6f' % train_loss_fenzi,'%.6f' % val_loss_fenzi,\
                  '%.6f' % train_loss_fenmu,'%.6f' % val_loss_fenmu,'%.6f' % train_loss_wrap,'%.6f' % val_loss_wrap,\
