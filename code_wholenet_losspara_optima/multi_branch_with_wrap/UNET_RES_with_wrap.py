...
class WHOLE_NET(nn.Module):
    def __init__(self):
        ...
        self.wrapped = UNet(1, 1)
        self.unwrapped = UNet(2, 1)
            

    def forward(self, x):


        x_wrapped = self.wrapped(x)
        xfusion2= torch.cat([x,x_wrapped], dim=1)
        x_unwrapped = self.unwrapped(xfusion2)

        return   x_wrapped,x_unwrapped
...
#   model=WHOLE_NET()
#   net = model.to(device)
#   x_wrapped,x_unwrapped= net(input_tensor) 
