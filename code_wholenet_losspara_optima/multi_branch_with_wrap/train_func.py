...
def train_net(net, device, loader, optimizer, loss_l1,loss_l2, w1,w2,w3,w4,batch_size):
    ...
    for batch_idx, (input,gt_fenzi,gt_fenmu,gt_wrap,gt_unwrap ) in enumerate(loader):
        input,gt_wrap,gt_unwrap = input.to(device), gt_wrap.to(device),gt_unwrap.to(device)                                                 # Send data to GPU
      #  print("input",input.size(),"gt",gt.size())
        output_train_wrap,output_train_unwrap = net(input) # Forward
        ...
        loss=+loss_wrap*w3+loss_unwrap*w4
...
#3sigma准则
    ...
    return train_loss_wrap.avg,output_train_wrap,\
...
def val_net(net, device, loader,loss_l1,loss_l2,w1,w2,w3,w4,batch_size):
    ...
    with torch.no_grad():
        for batch_idx, (input,gt_fenzi,gt_fenmu,gt_wrap,gt_unwrap ) in enumerate(loader):
            input,gt_wrap,gt_unwrap = input.to(device), \
            gt_wrap.to(device),gt_unwrap.to(device)                                                 # Send data to GPU
            output_val_wrap,output_val_unwrap = net(input) # Forward
            ...
            loss=+loss_wrap_val*w3+loss_unwrap_val*w4
...
#计算3sigma准则评价
    ...
    return val_loss_wrap.avg, output_val_wrap,\
