...
def train_net(net, device, loader, optimizer, loss_l1,loss_l2, w1,w2,w3,w4,batch_size):
    ...
    for batch_idx, (input,gt_fenzi,gt_fenmu,gt_wrap,gt_unwrap ) in enumerate(loader):
        input,gt_fenzi,gt_fenmu,gt_unwrap = input.to(device), gt_fenzi.to(device),gt_fenmu.to(device) ,gt_unwrap.to(device)                                                 # Send data to GPU
      #  print("input",input.size(),"gt",gt.size())
        output_train_fenzi,output_train_fenmu,output_train_unwrap = net(input) # Forward
        ...
        loss=loss_fenzi*w1+loss_fenmu*w2+loss_unwrap*w4
...
def val_net(net, device, loader,loss_l1,loss_l2,w1,w2,w3,w4,batch_size):
    ...
    with torch.no_grad():
        for batch_idx, (input,gt_fenzi,gt_fenmu,gt_wrap,gt_unwrap ) in enumerate(loader):
            input,gt_fenzi,gt_fenmu,gt_unwrap = input.to(device), gt_fenzi.to(device),gt_fenmu.to(device),\
            gt_unwrap.to(device)                                                 # Send data to GPU
            output_val_fenzi,output_val_fenmu,output_val_unwrap = net(input) # Forward
            ...
            loss=loss_fenzi_val*w1+loss_fenmu_val*w2+loss_unwrap_val*w4
